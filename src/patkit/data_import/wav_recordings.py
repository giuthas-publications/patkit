...
_logger = logging.getLogger('patkit.wav')


def generate_wav_recording_list(
        directory: Path,
    ...
    wav_files = directory.glob('*' + SourceSuffix.WAV)
    base_paths = [wav_file.with_suffix('')
                  for wav_file in wav_files]
    basenames = [Path(path).name for path in base_paths]
    recordings = [
        generate_audio_recording(
...
def generate_audio_recording(
    ...
    wav_file = directory/(basename + SourceSuffix.WAV)
    meta = RecordingMetaData(
        prompt="",
        time_of_recording=os.path.getmtime(wav_file),
        participant_id=""
    )
